<template>
    <div>
        <b-form  @submit.prevent="editar()" v-if="!formularioState">
            <b-card
            header="Editar Cliente Nuevos"
            >
            <b-alert v-model="showDismissibleAlert" variant="success" dismissible show>
                Se Editaron los datos Correctamente del Cliente
            </b-alert>
            ...
            <b-button variant="success" block class="mt-4" type="submit">Guardar</b-button>
            <b-button variant="danger" block class="mt-4" type="submit" @click="cancelarEditar()">Cancelar</b-button>
            </b-card>
        </b-form>
...
export default {
    ...
    computed: {
        ...
        ...mapState(['formularioState','cliente'])
       
    },
    methods: {
        ...mapMutations(['editarCliente','cancelarEditar']),
        editar(){
